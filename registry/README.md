...
CI 自动校验：schema、id 唯一、semver 递增、清单与索引一致、sha256 匹配、危险模式扫描，以及源码静态检查——颜色字面量（须用 `var(--nm-*)`）、无界循环字面量（`while (true)` / `for (;;)`）、远程资源引用（`https://` 出现在 `src=` / `@import` / `importScripts`；`host.fetch` 的 URL 不算）、图标按钮缺 `aria-label`（仅静态抓 HTML 字符串形式，`createElement` 命令式构建扫不到）、`@motion animated` 缺可见性响应、源码体积上限（256 KiB）。命中即拒绝安装。
