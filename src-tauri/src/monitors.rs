...
pub fn position_target<R: tauri::Runtime>(
    ...
    window
        .set_position(tauri::Position::Physical(*selected.position()))
        .map_err(crate::error::CommandError::system)
}
...
pub fn list_monitors(
    window: tauri::Window,
) -> Result<Vec<MonitorInfo>, crate::error::CommandError> {
    let primary = window
        .primary_monitor()
        .map_err(crate::error::CommandError::system)?;
    let monitors = window
    ...
    Ok(monitors
        ...
        .map(|monitor| {
            ...
            let is_primary = primary.as_ref().is_some_and(|value| {
...
mod tests {
    ...
    fn named_monitor_uses_the_os_name_as_id() {
        assert_eq!(
            monitor_id(Some("\\\\.\\DISPLAY2"), 1920, 0),
            "\\\\.\\DISPLAY2"
        );
    }
    #[test]
