...
- 铺满自身透明原生窗口的浮层（顶部状态岛与其详情面板）一律 `shadow-none`：外阴影会被窗口边缘裁掉，只在圆角外侧残留灰斑，层级改由 `1px` 边框承担。
...
- 虚线边框只用于上传区、空状态、可添加区域、顶部状态岛家族，或目标网页中同类语义，不用于普通信息卡片。
- 顶部状态岛（Home Indicator / 状态岛 / 其详情面板）是虚线边框的唯一浮层例外：状态岛 hover 时边框转为 `1px dashed #DAD3C3`，其详情面板常态即 `1px dashed #EAEAEA`。语义是「这块面板可被用户拿起并沿屏幕顶边移动」，与上传区同属「可操作容器」而非普通信息卡片。该例外不得扩展到其它卡片、弹窗、下拉菜单或模块容器。
...
顶部状态岛的长按拖动沿屏幕 X 轴跟手移动原生窗口，属于上表「直接响应输入的运动」，不是上述进出场例外：它没有补间，松手即停在指针所在处，并被工作区左右边界钳制。

顶部轨道浮层的展开与收起是第三项限定例外。状态岛、Nowly 入口与详情面板是**同一个物体的不同尺寸**：收起时它裂成胶囊与圆点两瓣，展开时合并为一张面。不得表现为另一个独立弹出的框。约束如下：

- 只允许动画 `width`、`height` 与 `opacity`；禁止位移、缩放、旋转、圆角跳变、阴影变化与任何装饰性补间。
- 展开 `220ms`，收起 `160ms`，缓动统一 `cubic-bezier(.22, 1, .36, 1)`。
- 面内内容按展开后的最终坐标定位，面长大时只能被「露出」，**禁止随尺寸重排或位移**；图标与标题在两种尺寸下必须落在同一像素。
- 只随展开状态切换的元素（信号徽标、两个位置的关闭键、Nowly 圆点）一律用 `opacity` 交叉淡变，禁止移动同一个元素。
- 面内内容淡入延迟 `90ms`、时长 `140ms`；淡出无延迟、时长 `110ms`。Nowly 圆点淡出延迟 `90ms`、时长 `130ms`，即等面覆盖上来之后再化掉。
- 原生窗口尺寸变化无补间：展开时先放大窗口再播动画，收起时先播完动画再缩小窗口。
- `prefers-reduced-motion: reduce` 下取消全部补间，仅保留即时显示与隐藏。

该例外不得扩展到 WebView 内的普通弹窗、下拉菜单、模块容器或主窗口任何控件。
