...
> 版本：1.0  
> 视觉基准：[Good – KeenThemes Dashboard](https://preview.keenthemes.com/good/?page=index)  
> 基准页面版本：Good 1.1.4 当前默认浅色主题  
> 数据来源：目标页面实际加载的 `style.bundle.css`，不是对截图的近似取色。
...
- 高度由内容与内边距形成，标准内边距：`12.4px 24px`（`0.775rem 1.5rem`）。
- 字号：`16px`。
- 字重：`500`。
- 行高：`1.5`。
- 圆角：`15.2px`。
...
- [ ] 已完整阅读最新的 `design.md`。
- [ ] 已确认没有沿用旧版 `#009EF7` 蓝色体系。
- [ ] 已复用公共 Token 和现有组件，而不是创建重复样式。

### 配色

- [ ] 主色为 `#4FC9DA`，激活色为 `#30A6B6`。
- [ ] 页面和卡片以白色、`#F8F6F2`、`#F6F1E9` 为主。
- [ ] 主文字为 `#211F1C`，次级文字来自规定暖灰阶。
- [ ] 普通边框为 `#EAEAEA`。
- [ ] 功能色使用语义正确且不只靠颜色传达信息。

### 字体

- [ ] 字体栈以 Inter 和系统中文无衬线字体为主。
- [ ] 标题、正文、辅助文字严格使用规范字号和字重。
- [ ] 正文基准为 `16px / 400 / 1.5`。
...
- [ ] 使用 4px 基础间距系统。
- [ ] 卡片之间通常为 `24px`。
- [ ] 标准卡片 Body 为 `32px 36px`，同级卡片保持一致。
- [ ] 未出现无依据的 13px、18px、22px、30px 等零散间距。
...
- [ ] 单日期字段使用 Good 离线日期选择器，包含 42 日网格、键盘和 ARIA 状态。
- [ ] 时间字段使用 Good 离线时间选择器，采用 24 小时制、5 分钟步长、Spinbutton ARIA 和规定键盘操作。
- [ ] focus-visible 清晰可见。
- [ ] 图标风格和尺寸统一。

### 动效

- [ ] 不存在 CSS transition 或 animation。
- [ ] 不存在 JS 动画、平滑滚动、自动轮播。
- [ ] 不存在 hover 位移/缩放、弹窗淡入、Spinner 或 Skeleton shimmer。
- [ ] 所有状态均即时切换。

只有以上项目全部通过，页面才符合 Nowly 统一设计规范。
