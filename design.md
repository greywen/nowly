...
### 2.0 Good 风格的背景分层（本次更新）

Good 默认首页采用“白色应用底 + 浅暖灰模块”的分层，而不是单一冷灰底色：

| 层级 | Token | 色值 | 用途 |
|---|---|---|---|
| 应用背景 | `bg-page` | `#FFFFFF` | 应用外层工作区与模块之间的留白 |
| 模块背景 | `bg-module` | `rgba(248, 246, 242, 0.3)` | 日历、四象限、便签、看板等模块主体，对齐 Good 卡片背景 |
| 表面背景 | `bg-surface` | `#FFFFFF` | 模块内部卡片、弹窗、下拉菜单 |
| 次级模块区 | `bg-secondary` | `#F6F1E9` | 分组底色、滚动条、今天/辅助提示 |
| 控件弱底 | `bg-subtle` | `#F8F6F2` | 输入框、工具栏、hover、空状态 |

应用背景只负责提供干净留白；模块主体使用 30% 透明度的浅暖灰形成轻微层级，内部卡片/弹窗再使用白色表面，通过 `1px solid #EAEAEA` 和统一 `15.2px` 圆角建立层级。顶部导航使用 Good 侧边栏的实色暖灰 `#F8F6F2`。
...
| Token | 色值 | 用途 |
|---|---:|---|
| `bg-page` | `#FFFFFF` | 页面基础背景、模块之间的留白 |
| `bg-surface` | `#FFFFFF` | 卡片、弹窗、下拉菜单、表格表面 |
| `bg-subtle` | `#F8F6F2` | 顶部导航、页面分区、输入框 solid 背景、hover 背景 |
...
字重对齐目标网页（Bootstrap 工具类 `fw-*`）。注意目标网页 `fw-semibold = 500`、`fw-bold = 600`、`fw-bolder = 700`，本表以实际渲染字重为准：

| Token | 字重 | 用途 |
|---|---:|---|
| `font-light` | `300` | 仅用于超大数字或装饰性轻文字（如时钟、专注计时） |
| `font-regular` | `400` | 正文、说明文本 |
| `font-medium` | `500` | 按钮、输入框、导航、标签 |
| `font-semibold` | `600` | 标题、卡片标题、重点文本、当前导航 |
| `font-bold` | `700` | 仅用于超大装饰性数字；正文标题一律不超过 `600` |

### 3.3 字号、字重与行高

目标网页根字号为 `html { font-size: 13px }`，即 `1rem = 13px`，正文基准行高 `1.5`，标题统一行高 `1.2`。**字号一律用 `rem` 表达，禁止写绝对 `px`**，让整站随根字号统一缩放。新页面按以下语义层级使用：

| 层级 | 字号 | 字重 | 行高 | 用途 |
|---|---:|---:|---:|---|
| Display | `1.75rem`（22.75px） | `600` | `1.2`（27.3px） | 仪表盘关键数字、少量大标题 |
| H1 / 页面标题 | `1.5rem`（19.5px） | `600` | `1.2`（23.4px） | 页面主标题 |
| H2 / 大模块标题 | `1.35rem`（17.55px） | `600` | `1.2`（21.06px） | 一级模块标题 |
| H3 / 卡片主标题 | `1.25rem`（16.25px） | `600` | `1.2`（19.5px） | 卡片或弹窗标题 |
| H4 / 小节标题 | `1.15rem`（14.95px） | `600` | `1.2`（17.94px） | 卡片内分组标题 |
| Body Large | `1.075rem`（13.975px） | `400–600` | `1.5`（20.96px） | 强调正文、主要列表项 |
| Body / 正文 | `1rem`（13px） | `400` | `1.5`（19.5px） | 默认正文、表单输入 |
| Body Small | `0.95rem`（12.35px） | `400–600` | `1.5`（18.53px） | 按钮、表格、次级正文 |
| Caption | `0.85rem`（11.05px） | `400–600` | `1.5`（16.58px） | 时间、标签、辅助说明 |

规则：

- 字号只能用上表的 `rem` 值，不得写绝对 `px`，也不得随意使用 `0.9rem`、`1.1rem` 等相近值替代。
- 标题统一 `600` 与行高 `1.2`；不得再用 `700` 承载普通标题。
...
- 禁止使用 2px 以上粗边框作为普通装饰；唯一例外是 §8.12 的 Tab 选中下划线（`3px`）。
...
- error：边框 `#F06445`，错误说明使用 `#F06445` 与 `0.85rem`。
...
- 布局：横向排列，标签之间间距 `32px`。
- **标签条本身不画底部基线**（目标网页对应 `border-bottom-0`），下划线只出现在选中标签下方。
- 标签条自身无背景色、无圆角、无内边距底色块。

标签项：

- 背景：`transparent`（任何状态下都不加背景块）。
- 内边距：`0 0 12px`，`margin-bottom: 0`。
- 字号：`0.95rem`，字重 `600`，行高 `1.5`。
- 下边框：`3px solid transparent`，未选中时占位但不可见，保证切换时不跳高。
- 默认文字：`#968E7E`。
- hover：文字 `#211F1C`，下边框仍为 transparent。
- 选中：文字 `#211F1C`，下边框 `3px solid #4FC9DA`（**只有下划线用主色，文字保持深色**）。
- disabled：文字 `#B5B0A1`，不响应交互。
- focus-visible：`0 0 0 4px rgba(79, 201, 218, 0.25)`。
- 计数后缀（如 `(47)`）：与标签文字同色同字重，仅用 `4px` 左间距分隔。

该 `3px` 下划线是 §7 中唯一允许的粗边框例外，仅用于 Tab 选中态；实测目标网页选中条为 `border-3`（约 `2.86px`，即 `3px`），不得改为 `1px` / `2px`，也不得额外补一条灰色基线。
...
快捷面板的 Windows 原生窗口进出场是另一项限定例外：面板仅可沿屏幕 Y 轴在顶部边缘与屏幕外之间移动，用于表达其固定空间来源；顶部 Home Indicator 与面板同步反向滑入或滑出。该例外不得扩展到 WebView 内容、普通弹窗或其他控件。
...
- 唯一入口是适配层 `src/components/icons.tsx`。字形数据由 `npm run icons`（`scripts/generate-icons.mjs`）从 `@iconify-json/solar` 生成到 `src/components/icon-data.ts`，该文件不得手改。
- 业务组件只能从适配层导入图标；不得直接引入图标库、不得新增内联 SVG、不得使用 Emoji 或远程 CDN 资源。仅品牌标识（如 GitHub）例外，因通用图标集不含。
- CC BY 4.0 要求署名，署名固定在「关于」弹窗。这些图标**不是** KeenIcons，文案与代码中不得声称是 KeenIcons。
- 更换图标集只需改生成脚本的来源并重新生成 `icon-data.ts`，业务组件无需改动。

### 11.2 三种风格

| 风格 | Solar 变体 | 用途 |
|---|---|---|
| `duotone` | `-bold-duotone` | 默认风格，对齐 Good 的 `ki-duotone` 基线 |
| `solid` | `-bold` | 实心 |
| `outline` | `-linear` | 线性 |

- 用户在「设置 → 界面 → 图标风格」切换，取值持久化在 `AppSettings.iconStyle`，由 `IconStyleProvider` 全局下发。
- 同一时刻全应用只使用一种风格；不得在同一界面混用两种风格，也不得混入卡通、Emoji 或异笔画图标。

### 11.3 尺寸与颜色

- 尺寸档位：`16px`、`18px`（默认）、`20px`。
- **图标不自己定颜色，一律继承所在控件的 `currentColor`**。同类控件无论在顶栏、模块工具栏还是卡片内，图标外观必须完全一致：

  | 场景 | 控件文字色 | 取值 |
  |---|---|---|
  | 默认按钮 | `--text-strong` | `#403D38` |
  | hover / 选中 | `--color-primary` | `#4FC9DA` |
  | 主操作按钮（`.btn-primary`） | — | `#FFFFFF` |
  | 低权重装饰图标（如拖拽手柄） | `--text-tertiary` | `#8E887A` |

- 禁止为某一区域单独覆盖图标的 `color` 或 `stroke-width`；需要更轻的图标时，改的是控件本身的文字色。
- 图标不得代替所有文字；不熟悉的操作必须提供文本或 Tooltip。
- 纯图标按钮固定 `40×40px`，必须提供 `aria-label`/`title`。
