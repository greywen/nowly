...
export function FocusTimerProvider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{const removers:Array<()=>void>=[];let disposed=false;const keep=(remove:()=>void)=>{if(disposed)remove();else removers.push(remove)};void listen<{id:string;plannedSeconds:number;startedAt:string}>('focus-timer-started',event=>{const startedAt=Date.parse(event.payload.startedAt);setState(startFocus(initialFocusState(event.payload.plannedSeconds/60),{id:event.payload.id,nowWallMs:Number.isNaN(startedAt)?Date.now():startedAt,nowMonoMs:performance.now()}))}).then(keep);void listen('focus-timer-paused',()=>setState(current=>pauseFocus(current,performance.now()))).then(keep);void listen('focus-timer-resumed',()=>setState(current=>resumeFocus(current,performance.now()))).then(keep);return()=>{disposed=true;removers.forEach(remove=>remove())}},[]);
