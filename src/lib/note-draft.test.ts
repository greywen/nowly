...
const note: Note = { id:'n1', title:'原则', content:'简单', color:'#4F55DA', pinned:true, createdAt:'2026-07-20', updatedAt:'2026-07-21' };

describe('note draft', () => {
  it('creates, copies, validates, and converts drafts', () => {
    expect(createNoteForm()).toEqual({ title:'', content:'', color:'#E8C444', pinned:false });
    expect(noteToForm(note)).toEqual({ title:'原则', content:'简单', color:'#4F55DA', pinned:true });
    ...
    expect(toNoteDraft({ ...createNoteForm(), title:'  原则  ', content:' 内容 ' })).toEqual({ title:'原则', content:' 内容 ', color:'#E8C444', pinned:false });
