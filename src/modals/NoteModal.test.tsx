...
const note={id:'n1',title:'产品原则',content:'简单',color:'purple' as const,pinned:true,createdAt:'x',updatedAt:'x'};
const base={onClose:vi.fn(),onSaved:vi.fn(),onDeleted:vi.fn(),createNote:vi.fn(),updateNote:vi.fn(),deleteNote:vi.fn()};
describe('NoteModal',()=>{
 it('validates and creates a fixed-color pinned note',async()=>{const user=userEvent.setup();const create=vi.fn().mockResolvedValue(note);render(<NoteModal mode={{type:'create'}} {...base} createNote={create}/>);expect(screen.getAllByRole('radio')).toHaveLength(6);await user.click(screen.getByRole('button',{name:'保存便签'}));expect(screen.getByText('请输入便签标题。')).toBeInTheDocument();await user.type(screen.getByLabelText('便签标题'),'产品原则');await user.click(screen.getByRole('radio',{name:'黄色'}));await user.click(screen.getByRole('checkbox',{name:'置顶便签'}));await user.click(screen.getByRole('button',{name:'保存便签'}));await waitFor(()=>expect(create).toHaveBeenCalledWith({title:'产品原则',content:'',color:'#FFC107',pinned:true}));});
