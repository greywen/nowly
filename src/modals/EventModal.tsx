...
import type { CalendarEvent, EventCategory, EventColor, EventDraft } from '../calendar/calendar-model';
...
const colors: {value:EventColor;label:string}[] = [{value:'blue',label:'蓝色'},{value:'red',label:'红色'},{value:'green',label:'绿色'},{value:'yellow',label:'黄色'}];

export function EventModal({ mode,tasks,restoreFocusRef,onClose,onSaved,onDeleted,createEvent,updateEvent,deleteEvent,now=()=>new Date() }:EventModalProps) {
  ...
  async function save(){
    ...
    setBusy(true);
    try { const draft=toEventDraft(form); const saved=mode.type==='create'?await createEvent(draft):await updateEvent(mode.event,draft); await onSaved(saved,mode.type==='edit'?mode.event.linkedTaskId:null); onClose(); }
  ...
  return <>
    <Dialog title={mode.type==='create'?'新建日程':'编辑日程'} ariaLabelledBy={titleId} isTopLayer={!confirm} restoreFocusRef={restoreFocusRef} onRequestClose={requestClose} className="event-dialog"
      ...
      <form className="event-form" onSubmit={e=>{e.preventDefault();void save();}}>
        ...
        <fieldset className="color-options"><legend>颜色</legend>{colors.map(color=><label key={color.value} className="form-check form-check-custom form-check-solid"><input className="form-check-input" type="radio" name="event-color" value={color.value} checked={form.color===color.value} disabled={busy} onChange={()=>update('color',color.value)}/><span className="form-check-label">{color.label}</span></label>)}</fieldset>{errors.color?<span className="field-error">{errors.color}</span>:null}
