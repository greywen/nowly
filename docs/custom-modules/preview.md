...
- **通道 A（给用户 / 开发期看真实 App）**：仪表盘上的常驻「开发者模块」（仅开发版可见），在里面选 `dev-modules/*.js` 草稿并按模块当前大小实时预览。开发模式（`tauri dev`）读仓库根的 `dev-modules/`（和通道 B 同一目录，写一次两处都看得到）；装机版读 app 数据目录（Windows 是 `%APPDATA%/com.nowly.app/dev-modules/`）。见文末「通道 A」一节。
...
装机版没有仓库、也没有 Vite 编译期的 `dev-modules/` 目录，但你仍想在**真实 App 环境**里看草稿。通道 A 就是为此存在：它不是一个弹窗，而是一个真正的**常驻仪表盘模块**——名叫「开发者模块」，像日历/便签一样能加到网格上、自由拖拽缩放。你把它拉多大，选中的草稿就按多大跑，等于在真实使用尺寸下、和其他模块并排预览。它只在**开发版**出现（`import.meta.env.DEV`，和后端 `cfg!(debug_assertions)` 对齐），装机用户的模块列表里看不到。
...
**不确定当前该往哪写？把「开发者模块」加到仪表盘，草稿为空时它会直接显示当前机器上的绝对路径**——AI 工具照抄即可。开发版里它显示的就是仓库 `dev-modules/`，装机版里显示的是 APPDATA 目录。

步骤：

1. 按上面的规则把草稿 `.js` 写到对应的 `dev-modules/`。
2. 在 App 里进入编辑模式 → 「添加模块」→ 内置区里点「开发者模块」把它加到网格上（只有开发版能看到）。
3. 模块顶部的下拉列出该目录下的草稿，选一个即在模块画布里实时预览，底部实时显示 lint。把模块拖大拖小就是在改预览尺寸。

和通道 B 的差别只有草稿来源：通道 B 靠 Vite 编译期把仓库 `dev-modules/*.js` 内联，通道 A 靠后端命令 `list_dev_modules` 在运行时读磁盘（开发版读仓库、装机版读 APPDATA）。沙箱、host、lint 完全一致，尺寸则由你在网格上把模块拖多大决定。

「开发者模块」每秒轮询一次该目录，改文件保存后无需任何手动操作即自动重挂——和通道 B 的「改完即见」一致。

调好后，用「添加模块」里的上传流程把草稿装成正式模块，它就出现在「我的模块」区，能被固定到仪表盘上——这条发布链路和普通用户一致。
