...
> **Spec C 完成**：五份实现计划（C1 存储 / C2 解析 / C3 调度 / C4a 读取渲染 / C4b 管理界面）全部落地。迁移 16 建 `calendar_subscriptions` + `external_events` 两表；`ics_parser` 手写最小 VEVENT 解析复用 Spec A 的 `rrule_engine` 展开；`subscription_sync` 拉取→解析→展开→整源替换写库→记状态，失败保留上次成功数据；后台线程启动即刷 + 每 60 秒按各源间隔检查到期。前端订阅事件合入月历（实心竖条 + 来源徽标 + 每源固定色）、只读详情弹窗、Settings「日历订阅」管理界面（列表 + 增删改 + 手动刷新 + 状态 + 最多 3 源）。325 个 Rust 测试 + 453 个前端测试全绿，`tsc` 与 `vite build` 通过。e2e（Playwright）需 Tauri 后端运行环境，按约定在此环境跳过。至此 Apple/Google/Outlook/Teams 导出的 `.ics`/`webcal://` 只读订阅全链路打通。
